...
class Memo(Base):
    ...
    course = Column(String(255))  # 長度
    assignment = Column(String(255))  # 長度
...
try:
    ...
    Base.metadata.create_all(engine)  # 創建
...
class MemoApp:
    def __init__(self, root):
        ...
        self.highlight_days_entry.insert(0, "3")  # 預設
    ...
    def load_memos(self, order='asc'):
        ...
        for memo in memos:
            ...
            if memo.due_date <= date.today() + timedelta(days=highlight_days):
                ...
                self.memos_listbox.itemconfig(tk.END, {'fg':'red'})  # 字體紅色
            else:
                self.memos_listbox.insert(tk.END, memo_text)
